...
class AgentRag:
    ...
    async def message(self, message: MessageEvent) -> List[Message]:
        ...
        sales_info = await agent.step(lookupSales, start_to_close_timeout=timedelta(seconds=120))
        ...
        completion = await agent.step(llm_chat, LlmChatInput(messages=self.messages, system_content=system_content), start_to_close_timeout=timedelta(seconds=120))
