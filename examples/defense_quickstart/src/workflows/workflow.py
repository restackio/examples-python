...
class RssWorkflow:
    ...
    async def run(self, input: dict):
        ...
        for content in crawled_contents:
            system_prompt = f"Provide a translation of the news article"
            user_prompt = f"Translate the following content: {content}"
            translation = await workflow.step(llm_chat, FunctionInputParams(system_prompt=system_prompt, user_prompt=user_prompt), task_queue="llm_chat",start_to_close_timeout=timedelta(seconds=120))

            system_prompt = f"Provide a summary of the news found on rss feed"
            user_prompt = f"Summarize the following content: {translation}"
        ...
        system_prompt = f"You are a personal assistant. Provide a summary of the latest news and the summaries of the websites. Structure your response with the title of the project, then a short description and a list of actionable bullet points."
        user_prompt = f"Here is the latest rss data: {str(rss_results)} and summaries of the websites: {str(summaries)}"
